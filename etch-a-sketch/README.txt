Excercise Etch-a-Sketch:

Create a webpage with a 16x16 grid of square divs.

Set up a “hover” effect so that the grid divs change color when your mouse
passes over them.

Add a button to the top of the screen which will clear the current grid and send
the user a popup asking for how many squares per side to make the new grid.
Once entered the new grid should be generated in the same total space as before
(e.g. 960px wide) and now you’ve got a new sketch pad.

(Optional): Instead of just changing the color of your grid have each pass
through it with the mouse change to a completely random RGB value. Then try
having each pass just add another 10% of black to it so that only after 10
passes is the square completely black.
